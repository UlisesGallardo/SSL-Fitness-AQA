...
def main(dataloaders, i3d, evaluator, args):
    ...
    for data in tqdm(dataloaders['test']):
        ...
        if args.TSA is False:
            for i in range(seg_number - 1):  # [4,3,200,224,224]=>[4,1024]
                start_frame = int(frames * i / (seg_number - 1))
                end_frame = int(frames * (i + 1) / (seg_number - 1))
                clip_feats[:, i] = i3d(videos[:, :, start_frame:end_frame, :, :], args).squeeze(2)
            clip_feats[:, seg_number - 1] = i3d(videos[:, :, -16:, :, :], args).squeeze(2)
        else:
            # ####
            # Stage 1 of I3D
            # ####
    ...
    #log_and_print(base_logger, '\t %s Acc: %.2f F1C0: %.2F F1C1: %.2f ' % (split, acc * 100, f1score_class_0, f1score_class_1))
    print('\t %s Acc: %.2f F-Score: %.2f  F1C0: %.2F F1C1: %.2f ' % ('Test',acc * 100, f1score_global, f1score_class_0, f1score_class_1))
