...
def get_mask(x, boxes, img_size):
    """
    :param x: [B, C, T*10, H, W]
    :param boxes:
    :param img_size: (H,W) == > (480,480)
    :return:
    """
    ...
    mask_b = np.array(mask_b)  # [B, 200, 14, 14]

    # Temporal pooling of mask  ==>  (B,T,H,W)  
    mask_final = []
    t_ratio = 4  

    for b in range(mask_b.shape[0]):  # Iterar sobre cada batch
        mask_crt = []
        for start_point in range(0, mask_b.shape[1], t_ratio):  # Iterar con paso t_ratio
            mask_clip = np.zeros((x.shape[3], x.shape[4]), dtype=np.uint8)
            for i in range(t_ratio):  # Iterar t_ratio frames
                mask_clip += mask_b[b, start_point + i]
            mask_crt.append(mask_clip)
        
        mask_final.append(mask_crt)

    return np.array(mask_final).astype(np.bool).astype(int)  # Devolver el resultado con las dimensiones (B, 16, H, W)
...
class MLP_block(nn.Module):
    ...
    def forward(self, x, dp_flag):
        ...
        #output = self.getprob(self.layer3(x)) #BCEWithlogits already has 
...
class Evaluator(nn.Module):
    ...
    def forward(self, feats_avg):  # data: NCTHW 
        probs = self.evaluator(feats_avg, dp_flag=True)  # Nxoutput_dim, args.dp
        return probs
